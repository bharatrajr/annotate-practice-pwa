...
const els = {
  ...
  filterBtn: $('filterBtn'), filterOverlay: $('filterOverlay'), filterCloseBtn: $('filterCloseBtn'),
  filterMatchCount: $('filterMatchCount'), filterBookmarkedToggle: $('filterBookmarkedToggle'),
  filterStatusChips: $('filterStatusChips'), filterResetBtn: $('filterResetBtn'),
  filterExamChips: $('filterExamChips'), filterSectionChips: $('filterSectionChips'), filterSubsectionChips: $('filterSubsectionChips'),
  filterSubjectChips: $('filterSubjectChips'), filterChapterChips: $('filterChapterChips'), filterTopicChips: $('filterTopicChips'),
  filterSubtopicChips: $('filterSubtopicChips'), filterDifficultyChips: $('filterDifficultyChips'),

  saveSessionBtn: $('saveSessionBtn'),
  fsToolChips: $('fsToolChips'), timerScaleSlider: $('timerScaleSlider'), fullscreenDefaultToggle: $('fullscreenDefaultToggle'),
  ghostPenSlider: $('ghostPenSlider'), ghostPenVal: $('ghostPenVal'), scrollingAnnotationToggle: $('scrollingAnnotationToggle'),
  scrollingAnnotationFsToggle: $('scrollingAnnotationFsToggle'),
  scrollAnnotateSpacer: $('scrollAnnotateSpacer'),
  fsSolutionBtn: $('fsSolutionBtn'),
...
const state = {
  ...
  bookmarks: new Set(),
  attempts: new Map(),   // questionId -> { selected: 'A', correct: true }
  optionOrderCache: new Map(), // questionId -> ['A','C','B','D'] slot order
  isFullscreen: false,
  filters: {
    exam: [], section: [], subsection: [], subject: [],
    chapter: [], topic: [], subtopic: [], difficulty: [],
    bookmarkedOnly: false, status: [], // status subset of: unanswered | answered | correct | incorrect
  },
  settings: {
    ...
    timerDefaultOn: false, annotateDefaultOn: false, fullscreenDefaultOn: false, timerOpacity: 1,
    timerScale: 1, fsAnnotateTools: [], ghostPenSeconds: 4, scrollingAnnotation: false, scrollingAnnotationFullscreen: false,
  },
  timer: {
  ...
  annotate: {
    ...
    strokes: [], redo: [],
    byQuestion: new Map(), // questionId -> strokes[]
    lastQid: null,
  },
...
let lastPenTs = 0;
// Two-finger-scroll-while-drawing: a second simultaneous touch means "pan",
// not "draw" — the in-progress single-finger stroke (if any) is dropped,
// and further movement of either finger scrolls the page (or the fullscreen
// card's internal scroll) instead of adding ink. Never triggered by a
// stylus — only real touch contacts count, so drawing with a pen while a
// palm rests nearby is unaffected.
const activeTouchPointers = new Map(); // pointerId -> last {x, y}
let panMode = false;
function twoFingerScrollBy(dy) {
  const useCardScroll = state.isFullscreen && !state.settings.scrollingAnnotationFullscreen;
  if (useCardScroll) {
    (state.showExplanation ? els.cardBack : els.cardFront).scrollBy(0, -dy);
  } else {
...
// ─────────────────────────────────────────────────────────────────────────
// Sessions: remembers files you've opened (their JSON + where you were in
// each one — current question, bookmarks, score, filters) so reopening the
// app resumes exactly where you left off. Stored in IndexedDB rather than
// localStorage because localStorage's ~5MB per-origin quota is easy to blow
// past with large Testbook JSON exports, which silently corrupted/dropped
// saved sessions. Auto-tracked sessions (last 8 files) evict oldest-first;
// manually-saved sessions ("Save as new session") are pinned and kept
// until you delete them, so you can hold onto multiple named checkpoints.
// ─────────────────────────────────────────────────────────────────────────
const IDB_NAME = 'mcq-annotate-db';
const IDB_STORE = 'sessions';
const MAX_AUTO_SESSIONS = 8;
...
async function idbPut(record) {
  try {
    ...
    return true;
  } catch (e) { return false; /* quota exceeded or unsupported — non-fatal */ }
}
...
// One-time migration: pull any sessions saved under the old localStorage
// scheme into IndexedDB, dropping entries that no longer parse cleanly
// (this also self-heals the "Could not find a question list" issue caused
// by localStorage quota corruption).
async function migrateLegacySessions() {
  const LEGACY_KEY = 'mcq_sessions_v1';
  let legacy = [];
  try { legacy = JSON.parse(localStorage.getItem(LEGACY_KEY)) || []; } catch (e) { legacy = []; }
  if (legacy.length) {
    for (const s of legacy) {
      try {
        detectRootArray(s.json); // validate shape before keeping it
        await idbPut(s);
      } catch (e) { /* corrupted entry — drop it */ }
    }
  }
  try { localStorage.removeItem(LEGACY_KEY); } catch (e) {}
}

async function initSessionsCache() {
  await migrateLegacySessions();
  const all = await idbGetAll();
  all.sort((a, b) => (b.savedAt || 0) - (a.savedAt || 0));
  state.sessionsCache = all;
}
...
function loadSessions() { return state.sessionsCache; }
function findSession(key) { return state.sessionsCache.find((s) => s.key === key) || null; }
function enforceSessionLimit() {
  const manual = state.sessionsCache.filter((s) => s.manual);
  const auto = state.sessionsCache.filter((s) => !s.manual);
  while (auto.length > MAX_AUTO_SESSIONS) {
    const removed = auto.pop();
    idbDelete(removed.key);
  }
  state.sessionsCache = [...manual, ...auto].sort((a, b) => (b.savedAt || 0) - (a.savedAt || 0));
}
function upsertSession(patch) {
  if (!state.activeSessionKey) return;
  const i = state.sessionsCache.findIndex((s) => s.key === state.activeSessionKey);
  const existing = i >= 0 ? state.sessionsCache[i] : { key: state.activeSessionKey };
  const merged = { ...existing, ...patch, savedAt: Date.now() };
  if (i >= 0) state.sessionsCache.splice(i, 1);
  state.sessionsCache.unshift(merged);
  enforceSessionLimit();
  idbPut(merged);
}
...
function annotationsSnapshot() {
  const map = new Map(state.annotate.byQuestion);
  if (state.annotate.lastQid != null) map.set(state.annotate.lastQid, state.annotate.strokes);
  return [...map];
}
function deleteSession(key) {
  state.sessionsCache = state.sessionsCache.filter((s) => s.key !== key);
  idbDelete(key);
  renderRecentList();
}
function clearAllSessions() {
  const keys = state.sessionsCache.map((s) => s.key);
  state.sessionsCache = [];
  keys.forEach((k) => idbDelete(k));
  renderRecentList();
}
function saveAsNewSession() {
  if (!state.currentRawJson) { showToast('Load a file first'); return; }
  const label = window.prompt('Name this session:', '');
  if (label === null) return;
  const trimmed = label.trim();
  if (!trimmed) return;
  const record = {
    key: `manual:${Date.now()}:${Math.random().toString(36).slice(2, 8)}`,
    name: trimmed, manual: true,
    json: state.currentRawJson, count: state.questions.length,
    lastQuestionId: currentQuestion()?.id ?? null,
    bookmarks: [...state.bookmarks], score: state.score,
    attempts: [...state.attempts], filters: state.filters,
    savedAt: Date.now(),
  };
  state.sessionsCache.unshift(record);
  idbPut(record);
  showToast(`Saved session "${trimmed}"`);
  renderRecentList();
}
function renderRecentList() {
  const sessions = state.sessionsCache;
  ...
  sessions.forEach((s) => {
    ...
    name.textContent = (s.manual ? '📌 ' : '') + (s.name || 'Untitled');
    ...
    del.textContent = '✕';
    del.title = 'Remove this session';
    del.addEventListener('click', (e) => { e.stopPropagation(); deleteSession(s.key); });
    row.appendChild(info); row.appendChild(del);
    row.addEventListener('click', () => {
      const ok = loadFromJSON(s.json, { name: s.name, session: s });
      if (!ok) {
        showLoadError(`"${s.name}" couldn't be loaded — its saved data looks corrupted, so it's been removed from this list.`);
        deleteSession(s.key);
      }
    });
...
const FILTER_FIELDS = ['exam', 'section', 'subsection', 'subject', 'chapter', 'topic', 'subtopic', 'difficulty'];
function cloneFilters(f) {
  const out = { bookmarkedOnly: !!f.bookmarkedOnly };
  FILTER_FIELDS.concat(['status']).forEach((k) => { out[k] = Array.isArray(f[k]) ? [...f[k]] : []; });
  return out;
}
const DEFAULT_FILTERS = cloneFilters({});
...
function loadFromJSON(json, opts = {}) {
  try {
    ...
    state.currentRawJson = json;

    const name = opts.name || 'Pasted JSON';
    const session = opts.session !== undefined ? opts.session : (opts.skipPersist ? null : findSession(sessionKeyFor(name, questions)));
    const key = session ? session.key : sessionKeyFor(name, questions);

    if (session) {
      state.bookmarks = new Set(session.bookmarks || []);
      state.score = session.score || { correct: 0, total: 0 };
      state.attempts = new Map(session.attempts || []);
      state.filters = cloneFilters({ ...DEFAULT_FILTERS, ...(session.filters || {}) });
      state.annotate.byQuestion = new Map(
        (session.annotations || []).map(([qid, strokes]) => [qid, strokes.filter((s) => s.tool !== 'ghost')])
      );
    } else {
    ...
    state.annotate.strokes = []; state.annotate.redo = []; state.annotate.lastQid = null;
    ...
    if (state.settings.fullscreenDefaultOn) setFullscreen(true);
    ...
    if (opts.skipPersist) {
      ...
      upsertSession({
        name, json, count: questions.length, manual: session?.manual || false,
        lastQuestionId: currentQuestion()?.id ?? null,
        bookmarks: [...state.bookmarks], score: state.score,
        attempts: [...state.attempts], filters: state.filters,
        annotations: annotationsSnapshot(),
...
// ─────────────────────────────────────────────────────────────────────────
// Filters — every field supports multiple selected values at once (OR within
// a field, AND across fields), and every field is cascading: each one's
// available chips are recomputed from whatever the *other* active filters
// currently allow, so narrowing one field updates what's selectable in all
// the others, in any direction — not just top-down.
// ─────────────────────────────────────────────────────────────────────────
function matchesStatus(q) {
  if (!state.filters.status.length) return true;
  const att = state.attempts.get(q.id);
  return state.filters.status.some((s) => {
    if (s === 'unanswered') return !att;
    if (s === 'answered') return !!att;
    if (s === 'correct') return !!att && att.correct;
    if (s === 'incorrect') return !!att && !att.correct;
    return false;
  });
}
function matchesFieldFilters(q, excludeField) {
  return FILTER_FIELDS.every((f) => {
    if (f === excludeField) return true;
    const selected = state.filters[f];
    if (!selected.length) return true;
    return selected.includes((q[f] || '').toString());
  });
}
function questionMatchesFilters(q) {
  return matchesFieldFilters(q, null)
    && (!state.filters.bookmarkedOnly || state.bookmarks.has(q.id))
    && matchesStatus(q);
}
function candidatesExcluding(field) {
  return state.questions.filter((q) =>
    matchesFieldFilters(q, field)
    && (!state.filters.bookmarkedOnly || state.bookmarks.has(q.id))
    && matchesStatus(q));
}
...
function populateFilterChips() {
  FILTER_FIELDS.forEach((field) => {
    const rowEl = document.querySelector(`.filter-field[data-field="${field}"]`);
    const container = els['filter' + field[0].toUpperCase() + field.slice(1) + 'Chips'];
    const candidates = candidatesExcluding(field);
    const values = [...new Set(candidates.map((q) => (q[field] || '').toString().trim()).filter(Boolean))]
      .sort((a, b) => a.localeCompare(b));

    state.filters[field] = state.filters[field].filter((v) => values.includes(v));

    if (values.length === 0) { rowEl.hidden = true; return; }
    rowEl.hidden = false;
    container.innerHTML = '';
    values.forEach((v) => {
      const btn = document.createElement('button');
      btn.className = 'chip-btn' + (state.filters[field].includes(v) ? ' active' : '');
      btn.type = 'button';
...
function syncFilterUI() {
  els.filterBookmarkedToggle.checked = state.filters.bookmarkedOnly;
  [...els.filterStatusChips.children].forEach((btn) => {
    btn.classList.toggle('active', state.filters.status.includes(btn.dataset.value));
  });
  updateFilterMatchCount();
}
function updateFilterMatchCount() {
  const n = state.questions.filter(questionMatchesFilters).length;
  els.filterMatchCount.textContent = `(${n} of ${state.questions.length})`;
}
function applyFilters() {
  ...
  syncFilterUI();
  buildOrder();
  const idx = prevQ ? state.order.findIndex((oi) => state.questions[oi].id === prevQ.id) : -1;
  state.currentPos = idx >= 0 ? idx : 0;
  renderQuestion();
  persistCurrentSession();
}
els.filterBtn.addEventListener('click', () => { populateFilterChips(); syncFilterUI(); els.filterOverlay.hidden = false; });
els.filterCloseBtn.addEventListener('click', () => { els.filterOverlay.hidden = true; });
els.filterOverlay.addEventListener('click', (e) => {
  const chip = e.target.closest('.chip-btn[data-field][data-value]');
  if (chip) { toggleFilterValue(chip.dataset.field, chip.dataset.value); return; }
  if (e.target === els.filterOverlay) els.filterOverlay.hidden = true;
});
els.filterBookmarkedToggle.addEventListener('change', () => {
  state.filters.bookmarkedOnly = els.filterBookmarkedToggle.checked;
  applyFilters();
});
...
function buildOrder() {
  const idxs = state.questions
    .map((_, i) => i)
    .filter((i) => questionMatchesFilters(state.questions[i]));
...
function renderQuestion() {
  const q = currentQuestion();
  swapAnnotationsTo(q ? q.id : null);
  const total = state.order.length;

  if (!q) {
    els.titleText.textContent = 'No matches';
    els.subText.textContent = '0 / 0';
    els.progressBar.style.width = '0%';
    els.fsListBtn.textContent = '0/0';
    els.sectionBadge.textContent = '';
    els.questionBody.innerHTML = 'No questions match the current filters. <button class="btn-link" id="noMatchResetBtn">Reset filters</button>';
    els.questionBody.style.fontSize = state.settings.fontSize + 'px';
    const resetLink = document.getElementById('noMatchResetBtn');
    if (resetLink) resetLink.addEventListener('click', () => { state.filters = { ...DEFAULT_FILTERS }; applyFilters(); });
    ...
    els.flipBtn.hidden = true; els.nextBtn.hidden = true;
    updatePrevNextState();
    return;
  }
...
function handleAnswer(letter) {
  ...
  state.attempts.set(q.id, { selected: letter, correct: isCorrect });
...
function toggleBookmark() {
  ...
  if (state.filters.bookmarkedOnly) {
    buildOrder();
    const idx = state.order.findIndex((oi) => state.questions[oi].id === q.id);
    state.currentPos = idx >= 0 ? idx : Math.min(state.currentPos, Math.max(state.order.length - 1, 0));
    renderQuestion();
  }
  persistCurrentSession();
}
...
function setFullscreen(on) {
  ...
  applyScrollingAnnotationMode();
}
// Options live inside the flashcard (single-screen, internally-scrolling
// layout) normally, but move back out to the page flow when fullscreen
// scrolling-annotation is on, since that mode reuses the normal-screen
// page-scroll layout (sticky question, everything else scrolls under it)
// rather than the fixed single-screen card.
function layoutForFullscreenMode() {
  const useCardLayout = state.isFullscreen && !state.settings.scrollingAnnotationFullscreen;
  if (useCardLayout) els.cardFront.appendChild(els.optionsWrap);
  else els.stage.insertBefore(els.optionsWrap, els.flipBtn);
}
...
els.fullscreenDefaultToggle.addEventListener('change', () => {
  state.settings.fullscreenDefaultOn = els.fullscreenDefaultToggle.checked;
  persistSettings();
});
els.timerScaleSlider.addEventListener('input', () => {
  state.settings.timerScale = Number(els.timerScaleSlider.value);
  els.timerWidget.style.setProperty('--timer-scale', state.settings.timerScale);
  persistSettings();
});
function applyFsAnnotateToolVisibility() {
  document.querySelectorAll('#annotateToolbar .full-only').forEach((el) => {
    const key = el.dataset.fskey || el.dataset.tool;
    el.classList.toggle('fs-visible-tool', state.settings.fsAnnotateTools.includes(key));
  });
  document.querySelectorAll('#annotateToolbar .tool-group').forEach((group) => {
    const hasVisible = [...group.children].some((c) => !c.classList.contains('full-only') || c.classList.contains('fs-visible-tool'));
    group.classList.toggle('fs-empty', !hasVisible);
  });
}
els.fsToolChips.addEventListener('click', (e) => {
  const chip = e.target.closest('.chip-btn[data-fstool]');
  if (!chip) return;
  const key = chip.dataset.fstool;
  const i = state.settings.fsAnnotateTools.indexOf(key);
  if (i >= 0) state.settings.fsAnnotateTools.splice(i, 1); else state.settings.fsAnnotateTools.push(key);
  chip.classList.toggle('active', i < 0);
  applyFsAnnotateToolVisibility();
  persistSettings();
});
function formatGhostSeconds(total) {
  if (total < 60) return total + 's';
  const m = Math.floor(total / 60), s = total % 60;
  return s ? `${m}m ${s}s` : `${m}m`;
}
els.ghostPenSlider.addEventListener('input', () => {
  state.settings.ghostPenSeconds = Number(els.ghostPenSlider.value);
  els.ghostPenVal.textContent = formatGhostSeconds(state.settings.ghostPenSeconds);
  persistSettings();
});
els.scrollingAnnotationToggle.addEventListener('change', () => {
  state.settings.scrollingAnnotation = els.scrollingAnnotationToggle.checked;
  persistSettings();
  applyScrollingAnnotationMode();
});
els.scrollingAnnotationFsToggle.addEventListener('change', () => {
  state.settings.scrollingAnnotationFullscreen = els.scrollingAnnotationFsToggle.checked;
  persistSettings();
  if (state.isFullscreen) layoutForFullscreenMode();
  applyScrollingAnnotationMode();
});
// Small "S" badge in fullscreen — reveals the solution directly without
// needing to tap through the (possibly ink-covered) question card, and
// works whether or not the question has been answered yet.
els.fsSolutionBtn.addEventListener('click', () => {
  const q = currentQuestion();
  if (!q) return;
  if (!q.explanation) { showToast('No explanation for this question'); return; }
  flipCard();
});
els.saveSessionBtn.addEventListener('click', saveAsNewSession);
...
function drawSegment(c, p0, p1, s, opacityMult = 1) {
  c.save();
  if (s.tool === 'eraser') { c.globalCompositeOperation = 'destination-out'; c.globalAlpha = 1; }
  else if (s.tool === 'highlighter') { c.globalCompositeOperation = 'source-over'; c.globalAlpha = 0.35 * opacityMult; }
  else { c.globalCompositeOperation = 'source-over'; c.globalAlpha = 1 * opacityMult; }
...
function drawSmoothSegment(c, p0, p1, p2, s, opacityMult = 1) {
  c.save();
  if (s.tool === 'eraser') { c.globalCompositeOperation = 'destination-out'; c.globalAlpha = 1; }
  else if (s.tool === 'highlighter') { c.globalCompositeOperation = 'source-over'; c.globalAlpha = 0.35 * opacityMult; }
  else { c.globalCompositeOperation = 'source-over'; c.globalAlpha = 1 * opacityMult; }
...
function drawDot(c, p, s, opacityMult = 1) {
  c.save();
  if (s.tool === 'eraser') c.globalCompositeOperation = 'destination-out';
  else if (s.tool === 'highlighter') c.globalAlpha = 0.35 * opacityMult;
  else c.globalAlpha = opacityMult;
  c.fillStyle = s.color;
...
// Ghost pen: strokes fade out and vanish a fixed time after being drawn
// (configurable in Settings). Fades over the last second of that duration.
function ghostOpacity(s) {
  const age = Date.now() - s.createdAt;
  const remaining = s.ghostDuration - age;
  if (remaining <= 0) return 0;
  const fadeWindow = Math.min(1000, s.ghostDuration);
  return remaining < fadeWindow ? remaining / fadeWindow : 1;
}
let ghostSweepInterval = null;
function startGhostSweep() {
  if (ghostSweepInterval) return;
  ghostSweepInterval = setInterval(() => {
    const hasGhost = state.annotate.strokes.some((s) => s.tool === 'ghost')
      || [...state.annotate.byQuestion.values()].some((arr) => arr.some((s) => s.tool === 'ghost'));
    if (!hasGhost) { clearInterval(ghostSweepInterval); ghostSweepInterval = null; return; }
    const before = state.annotate.strokes.length;
    state.annotate.strokes = state.annotate.strokes.filter((s) => s.tool !== 'ghost' || ghostOpacity(s) > 0);
    state.annotate.byQuestion.forEach((arr, qid) => {
      state.annotate.byQuestion.set(qid, arr.filter((s) => s.tool !== 'ghost' || ghostOpacity(s) > 0));
    });
    // Skip the repaint while a stroke is actively being drawn — redrawAll()
    // only knows about *finished* strokes, so painting mid-stroke would
    // wipe the live, not-yet-committed ink every tick and make new strokes
    // look laggy/delayed. It'll catch up the moment the finger lifts.
    if (!drawingPointer && state.annotate.active && !els.annotateLayer.hidden) redrawAll();
    if (state.annotate.strokes.length !== before) persistCurrentSession();
  }, 150);
}
function renderStroke(c, s) {
  if (s.tool === 'text') return drawText(c, s);
  if (s.tool === 'pen' || s.tool === 'highlighter' || s.tool === 'eraser' || s.tool === 'ghost') {
    const opacityMult = s.tool === 'ghost' ? ghostOpacity(s) : 1;
    if (opacityMult <= 0) return;
    const pts = s.points;
    if (pts.length === 1) return drawDot(c, pts[0], s, opacityMult);
    if (pts.length === 2) return drawSegment(c, pts[0], pts[1], s, opacityMult);
    for (let i = 2; i < pts.length; i++) drawSmoothSegment(c, pts[i - 2], pts[i - 1], pts[i], s, opacityMult);
    return;
  }
  drawShape(c, s);
}
// ─────────────────────────────────────────────────────────────────────────
// Scrolling annotation (normal screen only). The question card becomes
// sticky at the top; options and everything after it scroll normally
// underneath. A spacer below grows as you scroll near the bottom, giving
// you more room to write. It's capped rather than truly infinite — an
// uncapped canvas would demand more memory than most phones can give a
// single <canvas> (width × height × 4 bytes × devicePixelRatio²), so growth
// stops at SCROLL_ANNOTATE_MAX to avoid crashing the tab on long sessions.
// ─────────────────────────────────────────────────────────────────────────
const SCROLL_ANNOTATE_INCREMENT = 800;
const SCROLL_ANNOTATE_MAX = 3200;
function scrollingAnnotationOn() {
  return state.isFullscreen ? state.settings.scrollingAnnotationFullscreen : state.settings.scrollingAnnotation;
}
function applyScrollingAnnotationMode() {
  const on = scrollingAnnotationOn();
  document.body.classList.toggle('scroll-annotate-mode', on);
  els.scrollAnnotateSpacer.style.height = on ? SCROLL_ANNOTATE_INCREMENT + 'px' : '0px';
}
window.addEventListener('scroll', () => {
  if (!scrollingAnnotationOn()) return;
  const nearBottom = window.scrollY + window.innerHeight >= document.body.scrollHeight - 400;
  if (!nearBottom) return;
  const current = parseInt(els.scrollAnnotateSpacer.style.height || '0', 10);
  if (current >= SCROLL_ANNOTATE_MAX) return;
  els.scrollAnnotateSpacer.style.height = Math.min(SCROLL_ANNOTATE_MAX, current + SCROLL_ANNOTATE_INCREMENT) + 'px';
}, { passive: true });
...
// Annotations are kept per-question: save whatever's on the canvas under the
// question we're leaving, then load (or start fresh for) the one we're
// entering. Works whether annotate mode is currently visible or not, so
// switching questions never bleeds ink from one into another.
function swapAnnotationsTo(qid) {
  if (state.annotate.lastQid != null) {
    state.annotate.byQuestion.set(state.annotate.lastQid, state.annotate.strokes);
  }
  state.annotate.lastQid = qid;
  state.annotate.strokes = qid != null ? (state.annotate.byQuestion.get(qid) || []) : [];
  state.annotate.redo = [];
  if (scrollingAnnotationOn()) {
    els.scrollAnnotateSpacer.style.height = SCROLL_ANNOTATE_INCREMENT + 'px';
    window.scrollTo({ top: 0 });
  }
  if (state.annotate.active) resizeAnnotateCanvas();
}
...
els.annotateCanvas.addEventListener('pointerdown', (e) => {
  ...
  const rejected = palmRejected(e);
  if (e.pointerType === 'touch' && !rejected) {
    try { els.annotateCanvas.setPointerCapture(e.pointerId); } catch (err) {}
    activeTouchPointers.set(e.pointerId, { x: e.clientX, y: e.clientY });
    if (activeTouchPointers.size >= 2) {
      // A second finger just landed — this is a scroll gesture. Abort
      // whatever single-finger stroke the first finger may have started,
      // including wiping any of it that was already drawn live to the
      // canvas before it got saved.
      panMode = true;
  ...
  if (tool === 'pen' || tool === 'highlighter' || tool === 'eraser' || tool === 'ghost') {
...
els.annotateCanvas.addEventListener('pointermove', (e) => {
  ...
  if (e.pointerType === 'touch' && activeTouchPointers.has(e.pointerId)) {
    const prev = activeTouchPointers.get(e.pointerId);
    activeTouchPointers.set(e.pointerId, { x: e.clientX, y: e.clientY });
    if (panMode) {
      const dy = e.clientY - prev.y;
      if (dy) twoFingerScrollBy(dy);
      return;
    }
  }
  if (panMode) return; // another finger moving mid-pan — ignore for drawing
...
function endStroke(e) {
  if (e && e.pointerType === 'touch' && activeTouchPointers.has(e.pointerId)) {
    activeTouchPointers.delete(e.pointerId);
    if (activeTouchPointers.size < 2) panMode = false;
  }
  if (!drawingPointer) return;
  drawingPointer = false;
  if (state.annotate.tool === 'laser') { hideLaser(); currentStroke = null; return; }
  if (currentStroke) {
    if (currentStroke.tool === 'ghost') {
      currentStroke.createdAt = Date.now();
      currentStroke.ghostDuration = Math.max(1, state.settings.ghostPenSeconds) * 1000;
      startGhostSweep();
    }
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'Escape') {
    ...
    else if (!els.filterOverlay.hidden) els.filterOverlay.hidden = true;
...
async function init() {
  ...
  els.fullscreenDefaultToggle.checked = state.settings.fullscreenDefaultOn;
  els.timerOpacitySlider.value = Math.round(state.settings.timerOpacity * 100);
  els.timerWidget.style.setProperty('--timer-opacity', state.settings.timerOpacity);
  els.timerScaleSlider.value = state.settings.timerScale;
  els.timerWidget.style.setProperty('--timer-scale', state.settings.timerScale);
  els.ghostPenSlider.value = state.settings.ghostPenSeconds;
  els.ghostPenVal.textContent = formatGhostSeconds(state.settings.ghostPenSeconds);
  els.scrollingAnnotationToggle.checked = state.settings.scrollingAnnotation;
  els.scrollingAnnotationFsToggle.checked = state.settings.scrollingAnnotationFullscreen;
  applyScrollingAnnotationMode();
  [...els.fsToolChips.children].forEach((chip) => {
    chip.classList.toggle('active', state.settings.fsAnnotateTools.includes(chip.dataset.fstool));
  });
  applyFsAnnotateToolVisibility();
  applyTheme();
  buildAccentSwatches();
  ...
  //    skipping over any that turn out to be corrupted), 3) the load screen.
  fetch('questions.json', { cache: 'no-store' })
    .then((res) => (res.ok ? res.json() : Promise.reject()))
    .then((json) => loadFromJSON(json, { name: 'questions.json', silent: true }))
    .catch(() => {
      for (const latest of loadSessions()) {
        const ok = loadFromJSON(latest.json, { name: latest.name, session: latest, silent: true });
        if (ok) return;
        deleteSession(latest.key); // corrupted — drop it and try the next most recent
      }
    });
}
init();
