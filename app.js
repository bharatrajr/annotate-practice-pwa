...
const state = {
  ...
  settings: {
    ...
    timerScale: 1, fsAnnotateTools: [], ghostPenSeconds: 4, scrollingAnnotation: false,
  },
  timer: {
...
function applyScrollingAnnotationMode() {
  const on = state.settings.scrollingAnnotation && !state.isFullscreen;
...
window.addEventListener('scroll', () => {
  if (!state.settings.scrollingAnnotation || state.isFullscreen) return;
...
function swapAnnotationsTo(qid) {
  ...
  state.annotate.redo = [];
  if (state.settings.scrollingAnnotation && !state.isFullscreen) {
