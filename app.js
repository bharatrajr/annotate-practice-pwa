...
const els = {
  ...
  filterExam: $('filterExam'), filterSection: $('filterSection'), filterSubsection: $('filterSubsection'),
  filterSubject: $('filterSubject'), filterChapter: $('filterChapter'), filterTopic: $('filterTopic'),
  filterSubtopic: $('filterSubtopic'), filterDifficulty: $('filterDifficulty'),
...
const state = {
  ...
  isFullscreen: false,
  filters: {
    exam: 'all', section: 'all', subsection: 'all', subject: 'all',
    chapter: 'all', topic: 'all', subtopic: 'all', difficulty: 'all',
    bookmarkedOnly: false, status: 'all', // status: all | unanswered | answered | correct | incorrect
  },
  settings: {
...
const DEFAULT_FILTERS = {
  exam: 'all', section: 'all', subsection: 'all', subject: 'all',
  chapter: 'all', topic: 'all', subtopic: 'all', difficulty: 'all',
  bookmarkedOnly: false, status: 'all',
};
...
function loadFromJSON(json, opts = {}) {
  try {
    ...
    if (session) {
      ...
      state.filters = { ...DEFAULT_FILTERS, ...(session.filters || {}) };
    } else {
    ...
    if (opts.skipPersist) {
      ...
      upsertSession({
        name, json, count: questions.length,
...
// ─────────────────────────────────────────────────────────────────────────
// Filters — cascading: each dropdown's options are recomputed from whatever
// the *other* active filters currently allow, so picking e.g. a Section
// narrows which Subsections/Subjects/etc. show up, and vice versa.
// ─────────────────────────────────────────────────────────────────────────
function matchesStatus(q) {
  const att = state.attempts.get(q.id);
  switch (state.filters.status) {
    case 'unanswered': return !att;
    case 'answered': return !!att;
    case 'correct': return !!att && att.correct;
    case 'incorrect': return !!att && !att.correct;
    default: return true;
  }
}
function matchesFieldFilters(q, excludeField) {
  return FILTER_FIELDS.every((f) => {
    ...
    const val = state.filters[f];
    if (val === 'all') return true;
    return (q[f] || '').toString() === val;
  });
}
...
function populateFilterSelects() {
  FILTER_FIELDS.forEach((field) => {
    const selectEl = els['filter' + field[0].toUpperCase() + field.slice(1)];
    const rowEl = selectEl.closest('.filter-field');
    ...
    rowEl.hidden = false;
    if (state.filters[field] !== 'all' && !values.includes(state.filters[field])) state.filters[field] = 'all';
    const current = state.filters[field];
    selectEl.innerHTML = '<option value="all">All</option>' +
      values.map((v) => `<option value="${escapeAttr(v)}"${v === current ? ' selected' : ''}>${escapeAttr(v)}</option>`).join('');
  });
}
...
els.filterBtn.addEventListener('click', () => { populateFilterSelects(); syncFilterUI(); els.filterOverlay.hidden = false; });
els.filterCloseBtn.addEventListener('click', () => { els.filterOverlay.hidden = true; });
els.filterOverlay.addEventListener('click', (e) => { if (e.target === els.filterOverlay) els.filterOverlay.hidden = true; });
FILTER_FIELDS.forEach((field) => {
  const selectEl = els['filter' + field[0].toUpperCase() + field.slice(1)];
  selectEl.addEventListener('change', () => { state.filters[field] = selectEl.value; applyFilters(); });
});
els.filterBookmarkedToggle.addEventListener('change', () => {
  state.filters.bookmarkedOnly = els.filterBookmarkedToggle.checked;
  applyFilters();
});
els.filterStatusChips.addEventListener('click', (e) => {
